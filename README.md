...
| `tl` | **dual_smooth**, **call_clones**, **copy_number**, **segment** + **integer_cn** (segments → integer CN + purity/ploidy), **permutation_significance**; **`he_purity`** (H&E cellularity, `[he]` extra), **`cohort_compare`** (cross-sample), **`register`** (H&E↔grid, `[he]` extra) |
