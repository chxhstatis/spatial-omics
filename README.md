...
| `tl` | **dual_smooth**, **call_clones**, **copy_number**, **segment** (piecewise-constant CNA), **permutation_significance**; **`he_purity`** (H&E cellularity, `[he]` extra), **`cohort_compare`** (cross-sample), **`register`** (H&E↔grid, `[he]` extra) |
